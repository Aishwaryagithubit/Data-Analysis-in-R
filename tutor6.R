...
sms <- read.table("C:/Users/ASUS/Downloads/SMSSpamCollection.txt", sep="\t", stringsAsFactors = FALSE, quote="")
...
#To see the data, display ten columns for the first ten rows:
sms_converted<-read.csv("C:/Users/ASUS/Downloads/SMS_converted(1) (1).csv")
...
cm<-confusionMatrix(sms_test_pred, test$Type)
cm
# Task 6 Evaluate model nb_fit2. Can you see any difference in model performance between these models?
...
# Load the pROC library for ROC curve analysis (if not already loaded)
library(pROC)
